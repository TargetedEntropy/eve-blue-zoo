...
from apps.authentication.models import Characters, CharacterNotifications, SentNotifications, Users, SkillSet
...
class NotificationTasks:
    ...
    def send_discord_msg(self, user_id: str, character_name: str, skill_points: int) -> None:
        print("sending notification")
        dm_channel = discord_client.bot_request("/users/@me/channels", "POST", json={"recipient_id": user_id})
        return discord_client.bot_request(
            f"/channels/{dm_channel['id']}/messages", "POST", json={"content": f"Hello! {character_name} is ready to harvest with {skill_points:,}!"}
        )
    ...
    def should_notifications_be_sent(self, character_id):
            # reset notification
            with self.scheduler.app.app_context():
                notification_count = SentNotifications.query.filter(
                    SentNotifications.character_id == character_id
                ).filter(
                    SentNotifications.notification_cleared == False
                ).count()
    ...
    def main(self):
        ...
        for character in characters:
            ...
            if not self.should_notifications_be_sent(character.character_id): 
                ...
                # reset notification
                with self.scheduler.app.app_context():
                    notification_histories = SentNotifications.query.filter(
            ...
            if character.enabled_notifications == "sp-farm-notification":
                ...
                if character_skill_info.total_sp > 5500000:
                    print(f"{character_info.character_name} is ready for harvest with sp: {character_skill_info.total_sp:,}!")

                    # send discord message
                    self.send_discord_msg(user.discord_user_id, character_info.character_name, character_skill_info.total_sp)
