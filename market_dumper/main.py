...
from sqlalchemy import create_engine, Column, Integer, String, Float, Boolean, DateTime, BigInteger, Table
...
class MarketOrder(Base):
    ...
    # Price information
    price = Column(BigInteger, nullable=False)  # Using BigInteger for large price values
...
class BlueprintLongDurationOrder(Base):
    ...
    order_count = Column(Integer, nullable=True)  # Number of orders found with duration > 90
...
class EVEMarketCollector:
    def __init__(self, database_dsn: str = None):
        ...
        if not database_dsn:
            raise ValueError("DATABASE_DSN not found in environment variables or provided as argument")
    ...
    def get_null_faction_regions(self) -> List[int]:
        ...
        with self.Session() as session:
            regions = session.query(MapRegion.regionID).filter(
                MapRegion.factionID.is_(None)
            ).all()
    ...
    def fetch_market_orders(self, region_id: int) -> List[dict]:
        ...
        while True:
            ...
            try:
                logger.info(f"Fetching market orders for region {region_id}, page {page}")
                ...
                logger.error(f"Error fetching orders for region {region_id}, page {page}: {e}")
                break
        ...
        with self.Session() as session:
            ...
            for order_data in orders:
                try:
                    ...
                    existing_order = session.query(MarketOrder).filter_by(
                    ...
                    if existing_order:
                        ...
                        existing_order.issued = datetime.fromisoformat(order_data['issued'].replace('Z', '+00:00'))
                        ...
                        existing_order.price = int(order_data['price'] * 100)  # Store as cents to avoid float issues
                    else:
                        # Create new order
                        market_order = MarketOrder(
                            ...
                            price=int(order_data['price'] * 100)  # Store as cents to avoid float issues
                        )
                    ...
                    logger.error(f"Error saving order {order_data.get('order_id')}: {e}")
                    continue
            
            try:
                session.commit()
                logger.info(f"Saved/updated {saved_count} market orders for region {region_id}")
    ...
    def get_blueprint_type_ids(self) -> Set[int]:
        ...
        with self.Session() as session:
            ...
            blueprint_ids = session.query(IndustryActivityProduct.productTypeID).filter(
                IndustryActivityProduct.productTypeID.isnot(None)
            ).distinct().all()
    ...
    def find_long_duration_blueprint_orders(self):
        ...
        with self.Session() as session:
            ...
            long_duration_orders = session.query(MarketOrder.type_id).filter(
            ...
            for (type_id,) in long_duration_orders:
                ...
                existing = session.query(BlueprintLongDurationOrder).filter_by(
                    type_id=type_id
                ).first()
                
                if not existing:
                    # Count how many orders exist for this type
                    order_count = session.query(MarketOrder).filter(
            ...
            try:
                session.commit()
                logger.info(f"Saved/updated {saved_count} blueprint type IDs with long duration orders")
            except SQLAlchemyError as e:
...
def main():
    ...
    parser.add_argument('--get-regions', action='store_true', 
                      help='Get list of systems from MapRegion where factionID is NULL')
    parser.add_argument('--download-orders', action='store_true',
                      help='Download market orders for all NULL faction regions')
    parser.add_argument('--find-blueprints', action='store_true',
